...
**Last updated:** 2026-07-16 (Sprint 8: **8.1–8.8 DONE**, commits through **7b194a8**; 456 tests passing. Engine now reads a REAL Tata 1mg PDF end to end (word-geometry row reconstruction, `ocr/_rows.py`); clear finding phrasing (`medical/phrasing.py`); Diet + Lifestyle AI sections (8.6.1); confidence collapses to 0 on zero-parse. 8.7 = deploy config: first-class `demo_mode` setting (env `MEDISCAN_DEMO_MODE`, HARD override + locked UI toggle for the public Space), writable-RAG-cache fallback (`_resolve_cache_root`, #034), `docs/17-deploy-config.md` env contract. 8.8 = app-level E2E (`tests/integration/test_app_e2e.py`): drives the UI `analyze()` seam on the real cbc_report.pdf fixture -> rendered report + %PDF- bytes, offline (retriever stubbed), gated on pymupdf+weasyprint (RUNS on Mac, skips in sandbox). Next: **8.9 — evaluation pass** (extraction recall + the grading/scope review catalogued below), then 8.10 HF-Spaces deploy, 8.11 coverage ratchet, 8.12 close. macOS: `DYLD_FALLBACK_LIBRARY_PATH=/opt/homebrew/lib` in ~/.zshrc for WeasyPrint.)

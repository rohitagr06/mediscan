...
**Last updated:** 2026-07-16 (Sprint 8: 8.1–8.6.1 DONE, commits through **ca876d4**; 452 tests passing. Highlights: 8.3/8.4 WeasyPrint HTML→PDF (#035); 8.5 Gradio app + testable `analyze()` seam (#036); the MAJOR real-PDF fix (word-geometry row reconstruction in `ocr/_rows.py` → a real Tata 1mg PDF reads 33 assessed / 22 acknowledged; confidence collapses to 0 on zero-parse). 8.6 = clearer phrasing via shared `medical/phrasing.py::describe_finding` ('Creatinine is mildly elevated at 1.33'), quiet demo-mode chain log, unparsed dump collapsed behind a `<details>` accordion. 8.6.1 = NEW Diet + Lifestyle feature: `LifestyleConsideration` schema (informational-only), a 5th AI output (`LifestylePrompt`), enriched Diet prompt (Indian veg/non-veg food options), separate Diet / Lifestyle / combined-summary / Specialist render sections; demo mode stays generic, keyed runs are rich. VERIFIED end-to-end on Rohit's real report in KEYED mode (AI on) — summaries, diet, lifestyle, specialists all populate. Next: **8.7 — demo mode + deploy config** toward RC1 ship. Parser/scope items catalogued below for **8.9**.)
